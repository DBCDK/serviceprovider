...
**Testing is under major refactoring, this documentation will be updated, as soon as that has been done**
...
There is a script: `util/autotest.sh` that regenerates the unit tests. Make sure to start `npm run dev` first, and also to run `npm run prettier` afterwards.

Remember to check that the result of the request is as expected, as that will be included in the unit test as mock data.

The created unit test, has removed passwords etc. from the mocked data, which may lead to bugs in certain tests.
