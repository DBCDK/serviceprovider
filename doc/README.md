...
Then there are other snapshot tests that require a bit more work to regenerate, and thus cannot be fully autogenerated. They can be recognised by their `.snapshot.js` extension. Each of these can be recreated with `node util/autotest.sh recreate $FILENAME.snapshot.js`, with the following caveats depending on the test:
