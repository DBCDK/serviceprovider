...
Then there are other snapshot tests that require a bit more work to regenerate, and thus cannot be fully autogenerated. Each of these can be recreated with `node util/autotest.sh recreate $FILENAME.js`, with the following caveats depending on the test:
