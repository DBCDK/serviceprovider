...
import {
  getProfileActivity,
  getRelatedList
} from './utils/relatedEndpointMethods';
...
export function profile() {
  ...
  const router = createCRUD(
    ...
    Router(),
    map,
    getSchemaDefinition(swagger, 'Profile')
  );
  router.get(
    '/:id/likes',
    getRelatedList(
  ...
  router.get(
    ...
    getSingleProperty(
      ...
      'profile',
      usernameMap,
      getSchemaDefinition(swagger, 'UsernameExists')
    )
  );
  router.get(
    '/:profile_id/isFollowingGroup/:group_id',
    getSingleProperty(
      ['owner_id', 'entity_ref'],
      'action',
      followMap,
      getSchemaDefinition(swagger, 'UserIsFollowingGroup')
    )
  );

  router.get(
    '/:id/activity',
    getProfileActivity(map, getSchemaDefinition(swagger, 'Profile'))
  );
