...
describe('Automated test of the order endpoint', () => {
  it('expected response. ID:lo25jy, for {"pretty":true,"orderId":"non-existing","delete":true}', (done) => {
    ...
    provider.execute('order', {"pretty":true,"orderId":"non-existing","delete":true}, context)
      .then(result => {
      ...
      .catch(result => {
        fail({throw: result}, {"statusCode":500,"error":"Unknown order"});
        done();
      });
  });
});
