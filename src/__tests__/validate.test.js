    ...
    const expected = [
      {
        property: 'instance',
        message:
          'additionalProperty "test" exists in instance when not allowed',
        schema: {
          type: 'object',
          required: ['q'],
          ...
          properties: {
            ...
            createTest: {
              description:
                'name of test to create, or "random" for random name, or "mockfile" for writing mockfile.',
              type: 'string',
              example: 'random',
              noSwag: true
            },
            callback: {type: 'string', noSwag: true},
            q: {
            ...
            sort: {
              example: 'rank_title',
              description:
                'Order/ranking of results. This correspond to the `sort` parameter of opensearch, which, at the time of writing, can take values such as  `rank_title`, `rank_general`, `rank_main_title`, `rank_subject`, `rank_verification` (title and creator), `rank_creator`, `rank_none`, `date_descending`, `article_date_descending`, `acquisitionDate_descending`, and `random`.',
              type: 'string'
            },
            profile: {
              example: 'opac',
              description:
                "Optional search profile, it's passed directly to OpenSearch. In OpenSearch it's used to determine which works should be returned. The profile needs to be defined in VIP in order to use it.",
              type: 'string'
            }
          }
        },
        instance: {q: 'ost', test: 'hest'},
